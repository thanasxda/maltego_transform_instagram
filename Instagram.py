...
class Instagram(DiscoverableTransform):
    
    @classmethod
    def create_entities(cls, request: MaltegoMsg, response: MaltegoTransform):
        
        # obtain target info
        request_username = request.getProperty("username")
        ...
        for i in instagram_data:
            ...
            instagram_data_entity = response.addEntity("thanasxda.Instagram")
            
            # add properties to instagram entity
            response_username = i.get("username")
            if response_username:
                instagram_data_entity.addProperty("username", value = response_username)
